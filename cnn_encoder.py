...
# Normalize Pc and snwp separately
...
class ConvEncoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        dummy_input = torch.zeros(1, 1, X.shape[1], X.shape[1])
...
size = (X.shape[1] + 1) // 2
for i in range(100):
    pn_test = op.network.Demo(shape=[size, size, 1], spacing=1e-4)
    ...
    X_new = build_network_matrix(pn_test, Np=size)
    X_new_norm = (X_new - X_mean) / (X_std + 1e-8)
    X_new_tensor = torch.tensor(X_new_norm, dtype=torch.float32).unsqueeze(0).unsqueeze(0)

    with torch.no_grad():
        Y_pred_norm = model(X_new_tensor).squeeze().numpy()  # (130,)

    Y_pred    = Y_pred_norm.reshape(Y.shape[1], 2) * Y_std + Y_mean  # denormalize → (65, 2)
    pc_pred   = Y_pred[:, 0]
    snwp_pred = Y_pred[:, 1]
